...
$pdf->Cell(80);
$pdf->Cell(30, 4, 'Data Mata Kuliah', 0, 1, 'C');
$pdf->Ln(4);

// Header Tabel
$pdf->SetFont('Times', 'B', 11);
$pdf->Cell(15, 6, 'No', 1, 0, 'C');
$pdf->Cell(40, 6, 'kode Matkul', 1, 0, 'C');
$pdf->Cell(40, 6, 'Nama Matkul', 1, 1, 'C');


// Query Data Akademik
$query_panggil_matkul = mysqli_query($db, "SELECT * FROM tb_matkul") or die(mysqli_error($db));
$rv = mysqli_num_rows($query_panggil_matkul);

$pdf->SetFont('Times', '', 11);
if ($rv > 0) {
    $no = 1;
    while ($data = mysqli_fetch_array($query_panggil_matkul)) {
        $kode_matkul= $data['kode_matkul'];
        $nama_matkul   = ($data['nama_matkul']) ;


        $pdf->Cell(15, 6, $no++, 1, 0, 'C');
        $pdf->Cell(40, 6, $kode_matkul, 1, 0, 'C');
        $pdf->Cell(40, 6, $nama_matkul, 1, 1, 'C');
    }
}
