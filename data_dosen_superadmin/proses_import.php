...
if (isset($_POST['btn_import'])) {
    ...
    foreach ($data as $index => $row) {
        ...
        $nik     = mysqli_real_escape_string($db, trim($row[1]?? ''));
        ...
        if ($rv == 0) {
            mysqli_query($db, "INSERT INTO tb_dosen (nik, nama, kontak, email, kelamin) VALUES ('$nik', '$nama', '$kontak', '$email', '$kelamin')") or die(mysqli_error($db));
        }
    }
